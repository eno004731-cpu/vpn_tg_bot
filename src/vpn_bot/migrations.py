...
from vpn_bot.models import Invoice, Job, OneTimePlanPurchase, Subscription, User
...
async def _reset_postgres_sequences(session) -> None:
    if session.bind is None or session.bind.url.get_backend_name() != "postgresql":
        return
    for table_name in ("users", "invoices", "one_time_plan_purchases", "subscriptions", "jobs"):
